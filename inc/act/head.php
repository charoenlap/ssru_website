...
if(get('head_cat_id')){
	...
	if (!isset($_SESSION['head_cat_id'])) {
		$_SESSION['head_cat_id'] = 2249;
	}
}

// $head_cat = $obj_db->cat('parent = '.$_SESSION['head_cat_id'].$hide_del_lan_order)->rows;
// if (empty($head_cat)) {
// 	$head_cat = $obj_db->cat('parent = 1'.$hide_del_lan_order)->rows;
// }
// $con_setting = $obj_db->content('cat = '.$head_cat[6]['id'].$hide_del_lan_order)->rows;

$head_cat_id = $_SESSION['head_cat_id'];
...
$con_setting = $obj_db->content('cat = '.$head_cat[3]['id'].$hide_del_lan_order)->rows;

$color = $master_head_cat['color']; 
// print_r($con_setting);
...
<!-- style css -->
<!-- <link rel="stylesheet" href="assets/css/act/style.css?v=<?php echo time(); ?>"> -->
...
<style>
	...
	*, *:before, *:after {
	...
	.overlay-banner-two h1 {
	  font-size: 1.8rem;
	  color: #fff;
	  font-weight: bold;
	}
	.overlay-banner-two small {
	  font-size: 1.8rem;
	  color: #fff;
	}
	.btn-overlay {
	...
	.bg-course-one {
	  background-image: url(../../act/images/course1.jpg);
	  width: 100%;
	  height: 100%;
	  background-size: 100% 100%;
	  background-repeat: no-repeat;
